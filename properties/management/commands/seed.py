...
class Command(BaseCommand):
  ...
  def create_categories_and_types(self):
    ...
    for category_name, type_names in categories_and_types.items():
      category = PropertyCategory.objects.get_or_create(name=category_name)
      self.categories.append(category)
      for type_name in type_names:
        property_type = PropertyType.objects.get_or_create(name=type_name, category=category)
        self.types.append(property_type)

  def create_properties(self):
    titles = [
      'Spacious Apartment in Downtown', 'Modern House with Garden View', 'Luxury Villa near the Lake',
      'Central Office Space with City View', 'Prime Location Shop for Rent', 'Large Warehouse for Storage',
      'Scenic Plot of Land with Mountain Views', 'Fertile Agricultural Land for Sale', 'Industrial Land for Development'
    ]
